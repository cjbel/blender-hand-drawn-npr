...
\item The approach is assumed to require Python scripting to \creqman{265}{process render layers produced by the blender rendering engine}.
...
\Qitem{\Qq{Ref C120 and C190, it is assumed the the User will configure a Blender scene with a 3D surface mesh, including creation and positioning of a camera and any required lighting. This will be the starting point for interaction with the System. Please clarify if otherwise.} \Qlines{4}}
...
% \Qitem{\Qq{Ref C265, all rendering features are to be developed by taking an image-space processing approach. This requires the System to have access to appropriate render passes (e.g. direct diffuse, ambient occlusion, depth images). From investigation, it is not possible to access this internal render pass data directly via Blender's Python API. As such, these images must be first saved to disk, and then loaded by the System.}}
...
\begin{itemize}
\item A purely \areq{image-space} based approach will be taken, i.e. no inputs are taken from Blender's object-space.
\item \areq{It is not possible to obtain image-space data directly from Blender's render passes via the Python API}.
\item As a consequence of A20, \areq{Blender must save the required input images to disk before they can be processed by the System.}
\item As a consequence of A30, we define a new requirement: \areq{The System shall automatically activate required render passes and produce the required compositor node setup for saving these images to disk.}
\item \areq{It must be verified that the Surface Curvature Map and Stroke Orientation Map can be obtained from the Normal Map}.
\item \areq{The output (Final Rendering) shall be saved to disk, and will not be presented on-screen within Blender.}
\end{itemize}
